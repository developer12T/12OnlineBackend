...
const { addOrderErp } = require('../middleware/addOrderErp')
...
exports.getDataPrintReceipt = async function (list, action) {
  try {
    ...
    const idOrder = list
    if (action == 'UpdateInvoiceOrder') {

      const orderDatup = await Order.find({ cono: '1', id: { $in: idOrder } })
      ...
      if ((orderDatup <= 0)) {
        ...
        for (let i = 0; i < orderDatup.length; i++) {
          ...
          const inv12T = await Order.find(
            {},
            { invno: 1 }
          ).sort({ invno: -1 }).limit(1).lean()
        ...
        var numberser2 = await axios.post('http://apps.onetwotrading.co.th/online/api/M3API/OrderManage/Order/getNumberSeries', {
          series: 'ง',
          seriestype: '01',
          companycode: 410,
          seriesname: '071'
      ...
      const customerIdNumberList = [
        ...new Set(orderData.map(item => item.customerid))
      ]

      const customerData = await Customer.find({
        customerid: { $in: customerIdNumberList }
      }).lean()

      const shippingData = await Shipping.find({ shi_customerid: { $in: customerIdNumberList } })

      const customerMap = new Map(
        customerData.map(c => [c.customerid, c])
      )

      const shippingMap = new Map(
        shippingData.map(s => [s.shi_customerid, s])
      )

      const data = orderData.map(order => {
        const customer = customerMap.get(order.customerid) || null
        const shipping = shippingMap.get(order.customerid) || null
      ...
      const totalprint = await Order.find(
        { id: { $in: idOrder } },
        {
          totalprint: 1,
          statusprint: 1,
      ...
      if (action == 'lastRowActionToDataErp') {
        console.log('lastRowActionToDataErp')
      ...
      const resdata = [{
        ...
        printinv: data[0].printinv,
        list: listBuy,
        customer: data[0].customer,
        address: data[0].shippingstreetAddress,
      }]
      // console.log(resdata);
      // res.json(resdata);
...
const idOrder = ['13803516', '13783882']
const action = 'lastRowActionToDataErp'

exports.getDataPrintReceipt(idOrder, action)
